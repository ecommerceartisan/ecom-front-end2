...
export default function PrivateRoute() {
  ...
  useEffect(() => {
    // Asynchronously check for admin authentication by making an HTTP request.
    const authCheck = async () => {
      const res = await axios.get("/api/v1/auth/admin-auth");
      
      // If the response indicates the user is an admin, set 'ok' to true.
      if (res.data.ok) {
        setOk(true);
      } else {
        // If not an admin, set 'ok' to false.
        setOk(false);
      }
    };
