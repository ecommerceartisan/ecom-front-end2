...
const HomePage = () => {
  ...
  useEffect(() => {
    // When the component mounts, fetch categories and the total product count
    getAllCategory();
    getTotal();
  }, []);
  ...
  useEffect(() => {
    // When the page number changes, load more products
    if (page === 1) return;
    loadMore();
  }, [page]);
  ...
  useEffect(() => {
    // If no categories or price ranges are selected, fetch all products
    if (!checked.length || !radio.length) getAllProducts();
  }, [checked.length, radio.length]);

  useEffect(() => {
    // When categories or price ranges are selected, filter products
